...
Electron owns `$DSH_HOME/profiles/desktop`. Its `dependencies` contains only installed external plugins at exact registry versions or vendored `file:./desktop-plugins/*.tgz` specs; `dsh.profile.bundles` contains the built-in bundles followed by enabled plugins. The signed application supplies dsh, the private Desktop Host, and their production packages from `resources/dsh`. Shared package links resolve to those actual directories. Both host and plugins execute in the same bundled upstream Node process, with normal realpath resolution; Desktop does not enable `--preserve-symlinks`. The CLI cannot boot or mutate this profile.

The local startup page exposes startup status and available recovery actions; the loaded dsh renderer receives only the desktop protocol marker. The separate plugin window receives structured list, install, remove, update, and update-check operations. Install accepts a registry `name@version` spec or a local `.tgz`; Desktop copies the tarball into the profile and does not keep a dependency on the original path. Neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.
