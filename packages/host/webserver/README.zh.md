...
浏览器经由 `dsh-host-webserver` 通过 HTTP 访问 web GUI：一个 `node:http` 服务器，其他插件在其中注册具名路由、upgrade 路由、index 启动输入与一个回退 handler。它不了解任何 harness 概念，也不提供任何文件服务——`/api` 桥接、插件 bundle、HMR（热模块替换）事件流与 SPA dist 都属于注册它们的插件。路由匹配顺序固定不变：先在整张表中匹配精确 route，再匹配最长前缀，最后交给回退 handler。Electron 不绑定套接字（`listen: false`），而是通过 `fetchNamed` 分发这些具名路由。
...
把 webserver 组合为面向浏览器宿主的 HTTP 传输，然后让功能插件认领各自的路由。`listen` 为 true 时激活即开始监听；注册顺序不影响请求处理，因为具名路由组合起来互不相交。桌面端设置 `listen: false`，改用 `fetchNamed` 而不绑定端口。
