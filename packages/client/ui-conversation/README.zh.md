...
View 选择规则固定：有效且已注册的持久化选择优先，其次是已注册的 `chat`，否则不渲染 View；绝不选择第一个已注册 View。标签上的侧边打开控件会把第二个已注册 View 钉在主 View 右侧；composer 留在主栏。Shell phase 只组合 Session lifecycle 与 active-target set，不读取任何 target-specific 快照。
